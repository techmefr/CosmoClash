  ...
  return (
    <>
      <div className="icon_alliace">
        <MarsIco className="nav-icon" width={64} height={64} fillColor="#fff" />
      </div>
      <div className="info_alliace">
        <h1>{alliance?.name}</h1>
        <h4>
          Nombre de planetes:{" "}
          <span className="info_value">{alliance?.planet_count}</span>{" "}
        </h4>
        <h4>
          Nombre de jours:{" "}
          <span className="info_value">{alliance?.player_count}</span>{" "}
        </h4>
      </div>
    </>
  );
...
function SearchAlliance() {
  ...
  const getAllAliances = () => {
    ...
    return fetch(
      "https://localhost:8002/api/alliances/getNbPlayerAndPlanet",
      headers
    )
    ...
    <>
      <div className="flex items-center justify-center flex-col search-alliance-container">
        ...
        {alliances?.map((allian, index) => (
          <div
            ...
            className={
              "container_alliance " +
              (alliance && allian?.name === alliance?.name
                ? " container_alliance_selected"
                : "")
            }
            onClick={() => setAlliance(allian)}
          >
      ...
      <div id="alertBox" className="alert-box">
        <div className="alert-content">
          ...
          <button id="closeBtn" onClick={() => closeAlert()}>
            OK
          </button>
        </div>
      </div>
